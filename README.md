...
| `src/movies/movie/` | One Movie's own page, at `/movies/movie/?id=tt0068646`. Its full box office curve, its weekly takings, its facts and its ratings, plus the way back to any Campaign holding it. A section of the lookup rather than a top level address, which is what keeps `siteRoot` and the catch all's `<base>` bootstrap out of it. |
