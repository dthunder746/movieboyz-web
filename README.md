...
`404.html` renders any Campaign, draft or League landing path rather than only
the ones the build made a directory for. Only the current year and the one
League have directories, so the address everybody loads answers 200 and an
unlisted year or League answers 404 with a working page. See
...
| `src/draft/` | One Campaign year's draft page, at `/league/movieboyz/2026/draft/`. Three season tabs over a picks table, a leaderboard, a highlights strip and a sidebar of the Movies nobody took, plus a fourth Hits and Bombs tab holding the ten year long picks, and a what if mode that swaps picks around and re scores the board. Ported from the old site. `entry.js` is what the HTML loads and `layout.js` holds the page shell, but only down to an empty `#draft-app`: the tabs and every surface under them are built by `page.js` once the Campaign lands, which is how the old page worked and was left alone in the port. |
| `src/catchall/` | What `404.html` loads. It asks `route.js` whether the address names a draft, a Campaign or a League landing, and imports that page dynamically so the three stay in separate chunks. |
...
landing has its own directory plus the catch-all, and nothing in that shell
names a League, so a second League is reachable through the catch-all with no
deploy at all, and becomes a 200 with the same file in a directory named for it
plus one more build entry, rather than a second copy of the page to keep in
step.
...
a draft address differ by one segment on the end, a League landing address is a
Campaign address without its year, and the host has only ever one `404.html` to
answer all three with, so the catch all cannot load any page directly:
`src/catchall/entry.js` asks `route.js` which one the path names and imports it.
The import is dynamic so Vite splits the three, and a reader who landed on a
Campaign address does not download the draft page's what if mode to see it.
